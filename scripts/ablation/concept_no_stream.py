...
def generate_dataset1(id_key, all_question_trees, full_set_concepts, problem_feat_dict, concept_feat_dict, solution_feat_dict):
    features = []
    labels = []
    ids = []
    ...
    for id in tqdm(id_key, desc=f"处理进度:"):
        ...
        for c in random.sample(negative_concepts, len(negative_concepts)//2):
            # 拼接问题特征和概念特征（直接从字典获取）
            feat = torch.cat([problem_feat_dict[id], concept_feat_dict[c], solution_feat_dict[id]], dim=0)
...
def generate_dataset2(id_key, all_question_trees, full_set_concepts, problem_feat_dict, concept_feat_dict, solution_feat_dict):
    features = []
    labels = []
    ids = []
...
if __name__=="__main__":
    ...
    **{f"model.layers.{i}":  1 for i in range(48)}  # 所有层平分到每张卡
    ...
    solution_feats = extract_features(base_model, tokenizer, solution_texts_all, use_tqdm=True)
    solution_feat_dict = {id: feat for id, feat in zip(problem_ids_all, solution_feats)}
    ...
    for fold, (train_indices, val_indices) in enumerate(kf.split(unique_key)):
        ...
        train_dataset = generate_dataset1(
            ...
            problem_feat_dict, concept_feat_dict, solution_feat_dict
        )
