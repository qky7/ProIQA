...
if __name__ == "__main__":
    ...
    for id in tqdm(unique_key):
        ...
        all_trees[id] = ReasoningTree(questions[id]['reasoning_tree'],base_model,tokenizer)
    ...
    for fold, (train_indices, val_indices) in enumerate(kf.split(unique_key)):
        ...
        GNNModel = ReasoningTreeGNN(
            input_dim=2*base_model.config.hidden_size,
            graph_hidden_dim=base_model.config.hidden_size, 
            output_dim=base_model.config.hidden_size,
            dropout=gnn_dropout,
        ).to(device)
        optimizer = torch.optim.Adam(list(classifier.parameters()) + list(GNNModel.parameters()), lr=learning_rate)
