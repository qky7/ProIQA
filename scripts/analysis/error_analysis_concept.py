...
if __name__=="__main__":
    ...
    **{f"model.layers.{i}":  1 for i in range(48)}  # 所有层平分到每张卡
    ...
    Dataset_filepath = f"{args.data_dir}/{Dataset}/{Dataset}.json"
    #Dataset_filepath = f'{args.data_dir}/{Dataset}/concept_items.json'
    
    judge = {}
    judge_path = f"../output/analysis_concept_{Dataset}.json"
    ...
    print("预计算所有推理树特征...")
    tree_dict = {}
    for id in tqdm(unique_key, desc="预处理推理树"):
        tree_data = ReasoningTree(all_question_trees[id]['reasoning_tree'],base_model,tokenizer)
        tree_dict[id] = tree_data
    ...
    for fold, (train_indices, val_indices) in enumerate(kf.split(unique_key)):
        ...
        GNNModel = ReasoningTreeGNN(
            input_dim=2*dim,
            graph_hidden_dim=dim, 
            output_dim=dim,
            dropout=gnn_dropout,
        ).to(device)
            ...
            with torch.no_grad():  # 修正2：添加no_grad以加速推理
                for batch in val_loader:
                    ...
                    for i, id in enumerate(unique_ids):
                        ...
                        f1_list[id] = f1
                        traverse_pred_list = predicted[start:end]
                        traverse_true_list = batch_labels[start:end]
                        if id in judge.keys():
                            print("有误!!!")
                            exit()
                        judge[id] = {}
                        judge[id]['预测知识点'] = []
                        judge[id]['真实知识点'] = []
                        for j in range(len(traverse_pred_list)):
                            if traverse_pred_list[j] == 1:
                                judge[id]['预测知识点'].append(full_set_concepts[j])
                            if traverse_true_list[j] == 1:
                                judge[id]['真实知识点'].append(full_set_concepts[j])
